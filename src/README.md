...
- `app/nx-circular-chart/model/nx-circular-chart-model.ts` — `CircularChartConfig`/`CircularChartCardConfig`/`CircularChartSlice` (what the component renders), `CircularChartTypes` (Doughnut/Pie/SemiCircle enum), `DEFAULT_PALETTE`, the upstream widget payload shape (`RawCircularChartNode`/`RawCircularChartCollectionNode`), and the trend API response shape (`TrendGroup`/`TrendNode`/`TrendLeaf`/`TrendSeries`/`TrendDataPoint`).
