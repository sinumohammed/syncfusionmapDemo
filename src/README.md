...
- `app/nx-circular-chart/model/nx-circular-chart-model.ts` — `CircularChartConfig`/`CircularChartCardConfig`/`CircularChartSlice` (what the component renders), `CircularChartTypes` (Doughnut/Pie/SemiCircle enum), `DEFAULT_SERIES_PALETTE`/`SeriesPaletteEntry`/`SeriesPaletteShape` (color+shape legend/slice palette, overridable per-collection via `RawCircularChartCollectionNode.SeriesPalette`), the upstream widget payload shape (`RawCircularChartNode`/`RawCircularChartCollectionNode`), and the trend API response shape (`TrendNode`/`TrendLeaf`/`TrendSeries`/`TrendDataPoint`).
...
- `app/nx-circular-chart/services/nx-circular-chart-config.service.ts` — `NxCircularChartConfigService.fetchTrendResponse(url)`, the one live `HttpClient` call this feature makes (see "ApiUrl live fetch" below); mirrors nx-map's own `NXMapConfigService` split.
...
- **Which cards exist at all** — `buildCircularChartConfigs()` drops any `Configuration[]` node whose `Name` has no matching leaf ANYWHERE in the trend response entirely (no card, no empty-ring placeholder) — the trend response is the final, authoritative list of which circular charts exist right now, not just a values source. A node that DOES have a matching leaf but whose leaf carries no usable `Series` still renders (as the empty/no-data state below) — that's a real "matched, no readings yet" case, distinct from "not in this response at all".
- **Slice data** (`data`) — for a matched node, the leaf's own `Series[]` wins outright whenever it has at least one usable series; otherwise falls back to the widget node's own hardcoded `Data` (a JSON-encoded `CircularChartSlice[]` string). Both empty renders the empty (no-data) state, still fully clickable (not blocked — an all-zero card is still a real, meaningful selection on the map's own green-marker path).
...
- **`sublayerIds`** (`RawCircularChartNode.Param1`/`Param2`/`Param3`) — when any of the three carry a value, they're taken (in that order, skipping absent/null/empty ones) as this card's `sublayerIds`, emitted verbatim by `onCircularChartSelected()` on click instead of the normalized `Name`/`Id` fallback. All three absent/empty leaves `sublayerIds` unset, falling through to that existing `Name`/`Id` fallback unchanged.
...
`RawCircularChartCollectionNode.ApiUrl`, when set, makes `NxCircularChartCollectionComponent` fetch the trend response itself (via `NxCircularChartConfigService.fetchTrendResponse()`) and use THAT, ignoring the `trendResponse` @Input entirely — an `ApiUrl` on the config always wins, whether or not a host also passes `trendResponse`. Refetches on any `rawConfig`/`trendResponse` input change (the same `ngOnChanges` trigger the non-`ApiUrl` path already used) — a `trendResponse`-only change still re-hits `ApiUrl`, even though that value is ignored either way. A request-token guard discards a stale response if a newer change supersedes it before the fetch resolves; a fetch failure logs a console error and renders with no trend data rather than leaving stale data on screen. `ApiUrl` absent/null keeps the original host-supplies-`trendResponse` behavior, no network call.

After the response applies (`applyConfigs()`), the component calls `ChangeDetectorRef.detectChanges()` explicitly — confirmed live this is required: without it, `circularCharts`/legend data updates correctly, but `NxCircularChartComponent`'s own `*ngIf`-gated views (its chart/empty-ring) stay stuck on their pre-fetch state even though its class bindings (e.g. `isEmpty`) do update. This only shows up on the async (`ApiUrl`) path — the synchronous `trendResponse`-only path never needed it.

## Chart shape (`CircularChartTypes`)

`Doughnut = 1`, `Pie = 2`, `SemiCircle = 3`. This Syncfusion version's own `AccumulationType` only distinguishes Pie/Funnel/Pyramid (no native Doughnut) — a doughnut IS a Pie series with a non-zero `innerRadius`, so Syncfusion's own `type` stays `"Pie"` regardless and Pie-vs-Doughnut is driven purely by `innerRadius` (`"0%"` for `CircularChartTypes.Pie`, else `config.innerRadius ?? "72%"`). SemiCircle renders as an ordinary Doughnut with `startAngle: 270`/`endAngle: 90` pinned (Syncfusion has no native SemiCircle series type).
